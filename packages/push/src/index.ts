 ...
 *
 * Mora num pacote compartilhado, não em apps/web/lib, porque o worker
 * também precisa mandar aviso (acompanhamento de contas a receber) e não
 * pode importar de dentro de outro app. Ver apps/worker/src/receivables-
 * followup.ts.
 *
 * Chegou a morar em apps/web/lib/push.ts e ser importado por
 * instrumentation.ts — o build quebrava com "Can't resolve 'https'" porque
 * o Next empacota instrumentation.ts sem externalizar os módulos nativos do
 * Node que o web-push (via https-proxy-agent) carrega mesmo sem usar proxy.
 * Um pacote fora do Next não tem esse problema.
 */
...
export type AvisoPush = {
  ...
  url?: string;
  /** Agrupa avisos da mesma conversa (ou do mesmo pedido) num só, em vez de empilhar. */
  tag?: string;
};
